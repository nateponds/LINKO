...
The live showcase is deployed at **`linko.nateponds.com`**. Pre-seeded demo accounts exist for every role in the system. Credentials are documented internally in `backend/seeds/dev_seed.sql`.

| Email | Role | What to try |
| --- | --- | --- |
| `buyer@linko.test` | Buyer | Discover wholesalers, place an order, view invoices |
| `wholesaler@linko.test` | Wholesaler | Manage products/inventory, accept orders, create shipments |
| `logistics@linko.test` | Logistics Coordinator | Manage branches/couriers, assign couriers to parcels |
| `courier@linko.test` | Courier | Update parcel tracking statuses through to delivered |
| `admin@linko.test` | Platform Admin | Admin dashboard: user + business management |

Additional test accounts (`buyer2`, `wholesaler2`, `bizswitch`, `courier2–8`) are also seeded for multi-business and ownership-isolation scenarios.
