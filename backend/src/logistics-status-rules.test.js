...
test("transition map rejects moves not on an edge", () => {
  ...
  // Departed is never terminal -- Returned is not an edge off it, even once the
  // return leg is triggered.
  assert.equal(canCourierSubmitTrackingStatus("Departed Branch", "Returned", true).allowed, false);
});

test("Delivery Failed edge is trigger-gated: retry when not triggered, return leg once triggered", () => {
  // return leg not yet triggered -> retry only
  assert.equal(canCourierSubmitTrackingStatus("Delivery Failed", "Out for Delivery", false).allowed, true);
  assert.equal(canCourierSubmitTrackingStatus("Delivery Failed", "Arrived at Branch", false).allowed, false);

  // return leg triggered -> return leg only; never auto-writes Returned
  assert.equal(canCourierSubmitTrackingStatus("Delivery Failed", "Arrived at Branch", true).allowed, true);
  assert.equal(canCourierSubmitTrackingStatus("Delivery Failed", "Out for Delivery", true).allowed, false);
  assert.equal(canCourierSubmitTrackingStatus("Delivery Failed", "Returned", true).allowed, false);
});

test("return leg locks to Out for Return then Returned once triggered", () => {
  // Forward journey: no Returned option, only checkpoints / last-mile.
  assert.deepEqual(courierAllowedNextStatuses("Arrived at Branch", false), ["Departed Branch", "Out for Delivery"]);
  // Return leg: branch arrival can only leave for the sender, then complete.
  assert.deepEqual(courierAllowedNextStatuses("Arrived at Branch", true), ["Out for Return"]);
  assert.deepEqual(courierAllowedNextStatuses("Out for Return", true), ["Returned"]);
  assert.deepEqual(courierAllowedNextStatuses("Out for Return", false), []);

  assert.equal(canCourierSubmitTrackingStatus("Arrived at Branch", "Out for Return", true).allowed, true);
  assert.equal(canCourierSubmitTrackingStatus("Out for Return", "Returned", true).allowed, true);

  // Return leg is a trap: no redelivery, no hub departure once triggered.
  assert.equal(canCourierSubmitTrackingStatus("Arrived at Branch", "Out for Delivery", true).allowed, false);
  assert.equal(canCourierSubmitTrackingStatus("Arrived at Branch", "Departed Branch", true).allowed, false);
  assert.equal(canCourierSubmitTrackingStatus("Arrived at Branch", "Returned", true).allowed, false);
  assert.equal(canCourierSubmitTrackingStatus("Out for Return", "Returned", false).allowed, false);
  assert.equal(canCourierSubmitTrackingStatus("Delivery Failed", "Out for Return", true).allowed, false);
  assert.equal(canCourierSubmitTrackingStatus("Departed Branch", "Out for Return", true).allowed, false);
  assert.equal(canCourierSubmitTrackingStatus("Arrived at Branch", "Out for Return", false).allowed, false);
});

test("Returned is reachable only from Out for Return on the locked return leg", () => {
  assert.equal(canCourierSubmitTrackingStatus("Out for Return", "Returned", true).allowed, true);
  // Never directly from a branch arrival.
  assert.equal(canCourierSubmitTrackingStatus("Arrived at Branch", "Returned", false).allowed, false);
  assert.equal(canCourierSubmitTrackingStatus("Arrived at Branch", "Returned", true).allowed, false);
...
test("canSubmitTrackingStatus binds coordinators to the map, adds only Cancelled", () => {
  ...
  // Trigger-gated Delivery Failed edge applies to coordinators too.
  assert.equal(canSubmitTrackingStatus("Delivery Failed", "Arrived at Branch", true).allowed, true);
  assert.equal(canSubmitTrackingStatus("Delivery Failed", "Arrived at Branch", false).allowed, false);
});

test("courierAllowedNextStatuses matches the map and gates on the return trigger", () => {
  assert.deepEqual(courierAllowedNextStatuses("Out for Delivery"), ["Delivered", "Delivery Failed"]);
  assert.deepEqual(courierAllowedNextStatuses("Delivery Failed", false), ["Out for Delivery"]);
  assert.deepEqual(courierAllowedNextStatuses("Delivery Failed", true), ["Arrived at Branch"]);
  assert.deepEqual(courierAllowedNextStatuses("Arrived at Branch", true), ["Out for Return"]);
  assert.deepEqual(courierAllowedNextStatuses("Out for Return", true), ["Returned"]);
