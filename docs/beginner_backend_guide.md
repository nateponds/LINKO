...
1. **Single Language (JavaScript)**: React is written in JavaScript. Node.js lets you write your backend in JavaScript too. You do *not* need to learn Python, PHP, or Ruby!
...
- **`$1, $2, $3, $4` placeholders**: We never insert strings from the user directly into database queries (e.g. do *not* do `'INSERT ... VALUES (' + req.body.name + ')'`). That exposes the database to **SQL Injection** security hacks. Parameterizing variables with `$` placeholders keeps our queries safe.
...
app.get('/api/matching/suppliers', async (req, res) => {
  try {
    ...
    if (!buyerCity) {
      return res.status(400).json({ error: "buyerCity query parameter is required" });
    }
