...
export default function LogisticsManagementPage() {
  ...
  useEffect(() => {
    let active = true;
    apiGet("/api/logistics/decisions")
      .then((data) => {
        if (!active) return;
        setDecisions(data);
      ...
      .catch((error) => {
        if (active) setDecisionsError(error.message);
      });
    return () => { active = false; };
  }, []);
